...
path_WSE = ['./', REGION, '_03min_W3RA_AAU/WSE/'];  % WSE CaMa-Flood Outputs
path_Q = ['./', REGION, '_03min_W3RA_AAU/Q/'];      % Q (Discharge) CaMa-Flood Outputs
...
% DAHITI
file_d = dir(path_d);
path_d = [file_d(1).folder,'/'];
idx = ~vertcat(file_d.isdir);
file_d = {file_d.name}; file_d = file_d(idx);
if iscell(file_d)
...
% HYDROWEB
file_h = dir(path_h);
path_h = [file_h(1).folder,'/'];
idx = ~vertcat(file_h.isdir);
file_h = {file_h.name}; file_h = file_h(idx);
if iscell(file_h)
    NUM_SOURCES = NUM_SOURCES + 1;
...
% CLMS
file_c = dir(path_c);
path_c = [file_c(1).folder,'/'];
idx = ~vertcat(file_c.isdir);
file_c = {file_c.name}; file_c = file_c(idx);
if iscell(file_c)
    NUM_SOURCES = NUM_SOURCES + 1;
...
if NUM_SOURCES == 0
    error('No data from any resources were added!')
end
...
file_WSE = dir(path_WSE);
path_WSE = [file_WSE(1).folder,'/'];
idx = ~vertcat(file_WSE.isdir);
file_WSE = {file_WSE.name}; file_WSE = file_WSE(idx);
if iscell(file_WSE)
    fscmf.WSE = string();
...
file_Q = dir(path_Q);
path_Q = [file_Q(1).folder,'/'];
idx = ~vertcat(file_Q.isdir);
file_Q = {file_Q.name}; file_Q = file_Q(idx);
if iscell(file_Q)
    fscmf.Q = string();
